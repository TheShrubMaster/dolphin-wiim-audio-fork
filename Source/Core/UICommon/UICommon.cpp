...
void SetUserDirectory(std::string custom_path)
{
  ...
  //    -> Use GetExeDirectory()\User
  // 3. HKCU\Software\Dolphin Emulator\UserConfigPath exists and isn't the same as AppData\Roaming
  ...
  //        -> Use AppData\Local\Dolphin Emulator as the Application State directory path
  // 6. Default
  //    -> Use GetExeDirectory()\User
  ...
  else if (appdata_found)  // Case 5
  {
    ...
    if (localappdata_found)
    {
      application_state_path = TStrToUTF8(localappdata.get()) + DIR_SEP NORMAL_USER_DIR DIR_SEP;
    }
  ...
  else
  {
    ...
    else
    {
      ...
      if (File::Exists("/.flatpak-info") || !File::Exists(user_path))
      {
        ...
        std::string state_path = std::string(state_home && state_home[0] == '/' ?
                                                 state_home :
                                                 (home_path + ".local" DIR_SEP "state")) +
                                 DIR_SEP NORMAL_USER_DIR DIR_SEP;

        File::SetUserPath(D_USER_IDX, std::move(data_path));
        File::SetUserPath(D_CONFIG_IDX, std::move(config_path));
        File::SetUserPath(D_CACHE_IDX, std::move(cache_path));
        File::SetUserPath(D_APPLICATIONSTATE_IDX, std::move(state_path));
        return;
      }
    }
#endif
