...
void Interpreter::stswi(Interpreter& interpreter, UGeckoInstruction inst)
{
  ...
  Helper_StoreString(interpreter, inst, EA, inst.NB == 0 ? 32 : inst.NB, inst.RS);
}
...
void Interpreter::stswx(Interpreter& interpreter, UGeckoInstruction inst)
{
  ...
  Helper_StoreString(interpreter, inst, EA, u8(ppc_state.xer_stringctrl), inst.RS);
}

void Interpreter::Helper_StoreString(Interpreter& interpreter, UGeckoInstruction inst, const u32 EA,
                                     u32 n, u32 r)
{
  ...
  if (misalignment_bytes != 0)
  {
    ...
    current_value = interpreter.m_mmu.Read_U32(current_address, inst);
    if ((ppc_state.Exceptions & ANY_LOADSTORE_EXCEPTION) != 0)
      return;
  ...
  if (n != 0)
  {
    ...
    current_value &= 0xFFFF'FFFF'0000'0000;
    current_value |= (interpreter.m_mmu.Read_U32(current_address, inst) << (n * 8)) & 0xFFFF'FFFF;
    if ((ppc_state.Exceptions & ANY_LOADSTORE_EXCEPTION) != 0)
      return;
    interpreter.m_mmu.Write_U32(static_cast<u32>(current_value >> (n * 8)), current_address, inst);
  }
}
