...
WiimoteBase::WiimoteBase(const u8 index) : m_bt_device_index(index)
{
}
...
InputConfig* BalanceBoard::GetConfig() const
{
  return ::BalanceBoard::GetConfig();
}

Wiimote::Wiimote(const u8 index) : WiimoteBase{index}, m_config_index{index}
{
...
{
  return fmt::format("Wiimote{}", m_config_index + 1);
}
...
BalanceBoard::BalanceBoard(const u8 index) : WiimoteBase(index)
{
  ...
  // Feet
  groups.emplace_back(m_left_foot = new ControllerEmu::ControlGroup(_trans("Left Foot")));
  groups.emplace_back(m_right_foot = new ControllerEmu::ControlGroup(_trans("Right Foot")));
  for (auto& foot : {m_left_foot, m_right_foot})
  {
    // foot->AddInput(Translatability::Translate, "Lift");
    foot->AddInput(Translatability::Translate, "Step Off");
  }
  ...
  // Default is the on-center distance between the textured pads on the board.
  constexpr int DEFAULT_FOOT_SEPARATION = 27;
  m_options->AddSetting(&m_foot_separation_setting,
                        {_trans("Foot Separation"),
                         // i18n: The abbreviation for centimeters.
                         _trans("cm")},
                        DEFAULT_FOOT_SEPARATION, 0, 50);

  // Default is the length of the textured foot pad on the board.
  constexpr int DEFAULT_FOOT_LENGTH = 23;
  m_options->AddSetting(&m_foot_length_setting,
                        {_trans("Foot Length"),
                         // i18n: The abbreviation for centimeters.
                         _trans("cm")},
                        DEFAULT_FOOT_LENGTH, 0, 40);
...
ControllerEmu::ControlGroup* BalanceBoard::GetBalanceBoardGroup(BalanceBoardGroup group) const
{
  ...
  case BalanceBoardGroup::Balance:
    return m_balance;
  case BalanceBoardGroup::LeftFoot:
    return m_left_foot;
  case BalanceBoardGroup::RightFoot:
    return m_right_foot;
  default:
    ASSERT(false);
    return nullptr;
...
void BalanceBoard::UpdateBatteryStatus(double charge)
{
  ...
  m_status.battery_low = m_status.battery <= BalanceBoardExt::REFERENCE_BATTERY;
}
